...
% PARÁMETROS DE LA FUNCION EXPONENCIAL
...
% FUNCION A TROZOS DEL RADIO DEL CANAL
f_L = @(r) (exp(-0.5*((max(r,r_L).^2)/sig_L^2)))/ ...
    (exp(-0.5*(r_L/sig_L)^2));                       % Trozo para r<r_u
A = f_L(r_u)*exp(0.5*(r_u/sig_u)^2);                 % Continuidad
f_u = @(r) A*exp(-0.5*(r/sig_u).^2);                 % Trozo para r>=r_u

f = @(r) (r<r_u).*f_L(r) + (r>=r_u).*f_u(r);         % Funcion exponencial
