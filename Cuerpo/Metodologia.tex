...
\begin{equation}
  ...
  \symcal{E}_{x},
\end{equation}
con el símbolo $\triangleq$ tomando el significado de \enquote{representaciones equivalentes}.
...
d_{nm} = \Xval{n}{D}{m} = -e \int \varphi^{*}_{n}(\hat{\symbf{r}}_{a}) \hat{\symbf{r}}_{a} \varphi_{m}(\hat{\symbf{r}}_{a}) \diff^{3}\hat{r}_{a},
\end{equation}
...
De este forma, empleando tanto la ecuación \eqref{eq:3.14} como la ecuación \eqref{eq:3.15} pueden determinarse los dos términos del momento dipolar eléctrico.
