...
Combinando las ecuaciones \eqref{eq:3.1} y \eqref{eq:3.2a}, y \enquote{agrupando} las componentes que se propagan a lo largo de todo el eje $z$, se obtiene para el campo eléctrico
\begin{align}
...
\begin{equation}\label{eq:3.3}
  ...
  \end{split}
\end{equation}
donde $\laplacian_{\perp}$ contiene las derivadas parciales de segundo orden en las variables $x, y$.
...
\begin{listing}[htbp!]
  ...
  \inputminted[firstline=1, lastline=32]{python}{Programas/mlaser.py}
  \label{cod:3.1}
\end{listing}
...
\end{equation}
donde es importante observar, a partir de las ecuaciones \eqref{eq:3.11}, \eqref{eq:3.13b} y \eqref{eq:3.13b} que $\rho_{nm} = \rho^{*}_{mn}$ y, entonces, teniendo en cuenta que el operador $\symcal{D}$ es hermítico, $d_{nm} = d^{*}_{mn}$. Esta igualdad también puede obtenerse a través del producto escalar 
\begin{equation}\label{eq:3.15}
d_{nm} = \Xval{n}{\symcal{D}}{m} = -e \int \varphi^{*}_{n}(\hat{\symbf{r}}_{a}) \hat{\symbf{r}}_{a} \varphi_{m}(\hat{\symbf{r}}_{a}) \diff^{3}\hat{r}_{a},
\end{equation}
...
De este forma, empleando tanto la ecuación \eqref{eq:3.14} como la \eqref{eq:3.15} pueden determinarse los dos términos de la matriz dipolar.
Para terminar, combinando las ecuaciones \eqref{eq:3.8}, \eqref{eq:3.13a} y \eqref{eq:3.13b} se obtiene 
