...
\begin{equation}\label{eq:1.0}
  N_{2}/N_{1} =  \eu^{-(E_{2}-E_{1})/k_{B}T} < 1, 
\end{equation}
...
\begin{equation}\label{eq:2}
  \tcboxmath{\omega = \frac{E_{2}-E_{1}}{\hslash },}
\end{equation}
la probabilidad de emitir espontáneamente un fotón de frecuencia $\omega$ cuando un átomo evoluciona del estado de mayor a menor energía es conocida como \emph{coeficiente de emisión de espontánea} $A_{21}$. Por otro lado, la probabilidad de un fotón de inducir la transición $2 \rightarrow 1$ es proporcional al \emph{coeficiente de emisión estimulada} $B_{21}$ y a la densidad de energía espectral $\rho(\omega)$ de los fotones de frecuencia $\omega$ dentro de la cavidad. Estos coeficientes contribuyen a la variación de átomos en el nivel superior de la forma
\begin{equation}\label{eq:2.1}
  \diff N_{2} = -\left[A_{21} + B_{21} \rho(\omega)\right]N_{2} + \cdots .
\end{equation}
En cambio, la probabilidad de un fotón de inducir la transición $1 \rightarrow 2$ se denomina \emph{coeficiente de absorción} $B_{12}$, disminuyendo la población del nivel inferior e incrementando la del superior debido a la absorción en un factor $B_{12} \rho(\omega)N_{1}$. Como el número total de átomos es constante en este sistema, $\diff N_{2}+\diff N_{1}=0$, por tanto,
\begin{equation}\label{eq:2.2}
  \diff N_{2} = -\diff N_{1} = -\left[A_{21} + B_{21} \rho(\omega)\right]N_{2} + B_{12} \rho(\omega)N_{1}.
\end{equation}
La densidad $\rho(\omega)$ podría corresponderse con la energía de un haz láser penetrando a través del medio activo en el interior de una cavidad. En este punto, Einstein asume una situación de equilibrio de los átomos bajo la hipótesis del cuerpo negro. En equilibrio, las poblaciones son constantes, $\diff N_{1} = \diff N_{2} = 0$ y pueden calcularse a partir de una distribución de Boltzmann \autocite{Feynman2011}
\begin{equation}\label{eq:2.3}
  N_{1} = N \eu^{-\beta E_{1}}, \quad N_{2} = N \eu^{-\beta E_{2}},
\end{equation}
con $\beta = 1/k_{B}T$ y $N$ un factor para renormalizar la función de probabilidad asociada a ambos niveles a la unidad. Empleando la condición de equilibrio y la ecuación \eqref{eq:2.3} se tiene
\begin{equation}\label{eq:2.4}
  \left[B_{12}\eu^{-\beta E_{1}}-B_{21}\eu^{-\beta E_{2}}\right] \rho(\omega) = A_{21}\eu^{-\beta E_{2}},
\end{equation}
y despejando la densidad de energía
\begin{equation}\label{eq:2.5}
  \rho(\omega) = \frac{A_{21}}{B_{12}\eu^{\hslash \beta \omega} - B_{21}}.
\end{equation}
Sin embargo, el cuerpo negro en equilibrio tiene una densidad de energía \autocite{Milonni1988}
\begin{equation}\label{eq:2.6}
  \rho(\omega) = \frac{h}{\pi^{2}}\frac{\omega^{3}}{\eu^{\hslash \beta \omega} - 1},
\end{equation}
y la condición de equilibrio debe cumplirse para cualquier temperatura, por lo que las ecuaciones \eqref{eq:2.5} y \eqref{eq:2.6} implican que
\begin{align}
  \label{eq:2.7a}
  B_{12} &= B_{21}, \\
  \label{eq:2.7b}
  \frac{A_{21}}{B_{21}} &= \frac{h}{\pi^{2}} \omega^{3}.
\end{align}

Las implicaciones de estos resultados son considerablemente importantes. En primer lugar, la ecuación \eqref{eq:2.7a} dice que los coeficientes de absorción y emisión estimulada son iguales. Estos coeficientes pueden calcularse en mecánica cuántica \autocite{Sakurai2020} con relativa facilidad utilizando métodos perturbativos con un campo electromagnético externo, luego el coeficiente de emisión espontánea quedaría determinado. En segundo lugar, la ecuación \eqref{eq:2.7b} implica que para grandes frecuencias $\omega$, la emisión espontánea domina frente a la emisión estimulada, haciendo más difícil conseguir un láser para frecuencias muy elevadas.
