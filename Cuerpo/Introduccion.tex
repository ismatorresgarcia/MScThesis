...
En segundo lugar, los futuros lectores de este trabajo sentirán la necesidad de preguntarse por el título del mismo, su significado conjunto y el de los conceptos individuales que constituyen el estudio. La finalidad de este primer capítulo introductorio es satisfacer parcialmente estas preguntas acerca de la naturaleza del TFM para después, en los capítulos \S\ref{cap:2} y \S\ref{cap:3}, terminar de presentar las ideas básicas que subyacen el proyecto y comenzar el análisis de los resultados en el capítulo \S\ref{cap:4}. Sin embargo, para orientar anticipadamente a los lectores, se presentan a continuación ---sin entrar en detalle--- las líneas básicas que aparecerán más detalladas posteriormente:
...
\begin{itemize}
  ...
  \item Armónicos de alto orden. Aparecen nombrados en la literatura especializada como \acrshort{hoh}, acrónimo de \emph{\acrlong{hoh}}. La generación de armónicos de alto orden, también nombrada con el acrónimo \acrshort{hhg} (\emph{\acrlong{hhg}}), permite obtener armónicos de radiación coherente con duraciones extremadamente cortas (pueden llegar a obtenerse pulsos de attosegundos) mediante la interacción de un láser muy intenso con un blanco generalmente gaseoso o sólido. Aunque individualmente son fuentes de radiación \acrshort{xuv} coherente con propiedades ópticas de gran importancia, explicadas en la sección \S\ref{sec:1.3}, la inyección de armónicos de alto orden en amplificadores basados en plasmas densos permite obtener pulsos láser con características similares a los láseres de electrones libres (\acrshort{fel}), de mejores prestaciones que las demás fuentes por separado, pero reduciendo el tamaño y coste de las instalaciones. 
...
En los primeros años de desarrollo, en la década de $1960$, existía un gran escepticismo \autocite{SanchezRon2022} sobre su aparición, siendo muchas las personas que calificaban estas tecnologías emergentes como \enquote{una solución en busca de un problema}. Desde entonces han ido apareciendo muchos de estos problemas para los que el láser era la solución, como los ejemplos expuestos, hasta convertir el láser en una parte fundamental de la ciencia y la tecnología de nuestro tiempo.  

Las palabras láser y máser son acrónimos, respectivamente, de \emph{\acrfull{laser}} (amplificación de luz por emisión estimulada de radiación) y de \emph{\acrfull{maser}} (amplificación de microondas por emisión estimulada de radiación). Los orígenes teóricos de ambas técnicas, cuyos fundamentos aparecerán explicados durante la sección \S\ref{sec:1.1.1}, tienen como cimientos el contenido de dos artículos de Einstein\autocite{Einstein1916,Einstein1916a}, publicados en $1916$, acerca de la interacción de la radiación electromagnética con los niveles de energía de átomos, moléculas, etc.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{Figuras/ch1_intens_año.pdf}
...
La Figura \ref{fig:1.1} muestra esta evolución histórica, las distintas tecnologías y escalas de energía, así como las posibles tendencias y predicciones \autocite{Mourou2006} realizadas en $2006$. Mencionar que colaboraciones como ELI, situadas en Hungría, Rumanía y República Checa, o ILE en Japón, no han proporcionado todavía mediciones de intensidad en blancos, pero ambos proyectos podrían conseguir potencias del orden de $\sim \qty{10}{PW}$ en un futuro próximo, igualando e incluso superando posiblemente la intensidad del CoReLS (láser \ce{Nd{:}YAG} de \qty{4}{PW}). 
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item Emisión espontánea, en la que un sistema en un estado excitado pasa de manera espontánea a un estado inferior y emite un fotón en el proceso.
    \item Absorción, donde un fotón incidente es absorbido por un sistema, excitándolo.
    \item Emisión estimulada, en la que un sistema en un estado excitado es estimulado por un fotón, de forma que el sistema se desexcita emitiendo un fotón con el mismo estado que el fotón inicial.
...
Un sistema con coeficiente de absorción negativo se conoce como \emph{medio activo}. Este medio amplifica la radiación electromagnética resonante con sus niveles de energía mediante la emisión estimulada. La descripción clásica de este fenómeno supone la existencia del medio activo. Para comprender la naturaleza de estos medios, es necesario recurrir a la mecánica cuántica.
...
En cambio, en ocasiones puede presentarse la situación contraria en la naturaleza o en un laboratorio, encontrándose un grupo de átomos en una \emph{inversión de población} con $N_{2} > N_{1}$. En cierto modo, \enquote{se podría decir que el sistema tiene una temperatura negativa}. La luz que se propaga a través de medios con una inversión de población experimenta más emisión estimulada que absorción, amplificándose en el proceso. El resultado es la amplificación de luz por emisión estimulada de radiación, o efecto \enquote{láser}.
...
Una vez los electrones decaen hasta el estado $\ket{1}$, vuelven a decaer rápidamente hasta el estado fundamental para después, posiblemente, volver a ser bombeados devuelta hasta los estados de absorción. Este mecanismo es conocido como \enquote{bombeo de cuatro niveles} (\emph{four-level pumping} en inglés), mientras que si el estado $\ket{1}$ es el estado fundamental se denomina \enquote{bombeo de tres niveles} (\emph{three-level pumping}). Además, también suele hacerse una distinción especial con un tercer esquema denominado \enquote{bombeo de cuasi-tres niveles} (\emph{quasi-three level pumping}) cuando el estado inferior de la transición láser $\ket{1}$ tiene una energía ligeramente superior al estado fundamental. La sección \S\ref{sec:3.3} mostrará como, de hecho, el esquema empleado por el experimento estudiado en este proyecto es similar a este.

La idea básica consiste en que, si el sistema de bombeo es capaz de inyectar energía súbitamente, es decir, suficiente potencia (con otro láser, por ejemplo), este proceso produce una inversión de población temporal entre los estados $\ket{2}$ y $\ket{1}$, gracias al cual una ráfaga incidente, relativamente débil de luz o \enquote{semilla} puede interaccionar con estos estados emitiendo una ráfaga de luz amplificada. El resultado es un láser pulsado, como el simulado durante el capítulo \S\ref{cap:4}. En el caso de tener un sistema de bombeo que funcione de forma continua, podría conseguirse como resultado una inversión poblacional permanente mediante el cual las \enquote{semillas} de luz incidentes puedan interaccionar, produciendo una luz láser de onda continua, en lugar de pulsada.

Mientras que el láser viaja a través del medio activo (los electrones fuera del equilibrio, en inversión de población), el flujo de energía $F$ aumenta con la distancia $z$ tal que $\symrm{d}F/\symrm{d}z = F/l_{o}$, entonces $F(z) = F_{o}\eu^{z/l_{o}}$. Aquí, $F_{o}$ es el flujo inicial, y $l_{0} \equiv 1/|\mu|$ (longitud de amplificación en un factor $\symrm{e}$) depende de la intensidad de la inversión poblacional y de la intensidad del \enquote{acoplamiento} entre la luz y el medio activo. Normalmente, $l_{0}$ es demasiado grande como para conseguir un efecto láser suficientemente intenso en un solo recorrido de la luz por el medio activo. En estos casos, la amplificación hay que potenciarla introduciendo el medio activo en el interior de una \emph{cavidad de Fabry-Perot} (esquema en la Figura \ref{fig:1.4}), llamada frecuentemente \emph{cavidad resonante}.
...
Los pulsos láser pueden conseguirse activando y desactivando la fuente de bombeo, empleando la técnica conocida como \emph{mode-locking} (esencialmente consiste en inducir una interferencia destructiva entre ciertos modos de la cavidad de Fabry-Perot), o utilizando el esquema llamado \emph{Q-switching}. Este consiste en interrumpir el efecto láser, por ejemplo, introduciendo en la cavidad un material óptico que aumenta las pérdidas de la cavidad hasta que la fuente de bombeo haya producido una inversión de población suficientemente grande, para después reducir estas pérdidas, facilitando la amplificación.
...
\begin{equation}\label{eq:1.7}
  \rho(\nu) = \frac{8 \pi h \nu^{3}}{c^{3}}\frac{1}{\eu^{h \nu/(k_{B}T)} - 1},
\end{equation}
y la condición de equilibrio debe cumplirse para cualquier temperatura en el interior de la cavidad, por lo que comparando las ecuaciones \eqref{eq:1.6} y \eqref{eq:1.7} los coeficientes deben cumplir (en unidades del Sistema Internacional)\autocite{Milonni1988,Davis2014}
\begin{align}
  \label{eq:1.8a}
  B_{12} &= B_{21}, \\
  \label{eq:1.8b}
...
Además, este último hecho motiva la extrema dificultad de la amplificación de láseres de rayos X blandos, como mostrará la sección \S\ref{sec:1.4}, necesitando emplear métodos de bombeo con suficiente potencia para conseguir una inversión de población que \enquote{compense} las altas frecuencias de la radiación \acrshort{xuv}. 
...
Es importante remarcar que la ecuación \eqref{eq:1.9} solamente es correcta cuando los procesos de excitación--desexcitación colisionales y radiativos están en equilibrio. En la mayoría de plasmas producidos en un laboratorio, la absorción radiativa es débil debido al pequeño tamaño que ocupa el plasma, de forma que esta expresión matemática es válida cuando los procesos de excitación--desexcitación colisional dominan frente a los radiativos. 

Rápidamente, a partir de la ecuación \eqref{eq:1.9} se deduce que si las poblaciones están en equilibrio, es imposible tener una inversión de población entre ambos estados. Las relaciones entre los coeficientes de Einstein quedan como (en el Sistema Internacional de Unidades)\autocite{Milonni1988,Davis2014} 
...
\end{equation}
siendo $I_{av}$ la intensidad láser promediada con la función de línea, $g_{0}$ el coeficiente de ganancia pequeña para el centro del perfil y $I_{s}$ la intensidad de saturación del láser. 
...
\end{equation}
Para longitudes de onda en el rango óptico, por ejemplo, $\lambda = \qty{500}{nm}$, y con una superficie de salida de la cavidad habitual de $A = \qty{5}{mm^2}$, la relación \eqref{eq:1.3} proporciona una divergencia de $\Delta\theta = \sqrt{\qty{250000e-18}{m^2}/(\qty{25e-6}{m^2})} = \qty{0,1}{mrad}$. Con ángulos de divergencia similares a este ejemplo, que son habituales en láseres, como muestra la Tabla \ref{tab:1.1}, la llamada \emph{longitud de Rayleigh} permite caracterizar la distancia recorrida $z$. En este rango, el ancho $w_0$ del haz láser se incrementa en un factor $\sqrt{2}$ en el plano transversal, siendo aproximadamente en este caso
...
\begin{figure}[htpb]
  ...
  \caption{Patrones de difracción y densidad de electrones obtenida de un \enquote{Mimivirus}\autocite{Ekeberg2015}. La resolución conseguida es de \qty{125}{nm}. El tamaño de la cápside del virus es aproximadamente de \qty{450}{nm}.}
  \label{fig:1.9}
\end{figure}

El aspecto clave del ancho de pulso en estas aplicaciones está en conseguir la difracción antes destruir completamente la muestra\autocite{Neutze2000} (ver Figura \ref{fig:1.10}), motivando la necesidad de pulsos extremadamente cortos o ultracortos ($\le\unit{fs}$) para visualizar imágenes. Por otro lado, el papel de la longitud de onda es conseguir la resolución necesaria para observar menores tamaños, ya que longitudes de onda mayores que el tamaño característico del objeto que quiere estudiarse no exhiben el fenómeno de difracción necesario. En la actualidad ---se explicará más detalladamente en la sección \S\ref{sec:1.3}---, las únicas fuentes de radiación que reúnen estos requisitos son los láseres de rayos X blandos y los láseres de electrones libres.

\paragraph{Modos de Laguerre-Gauss}
Durante la sección \S\ref{sec:1.1.1}, emergió de forma natural el concepto de cavidad de Fabry-Perot o cavidad resonante. La función de estos sistemas ópticos es fundamental para conseguir una amplificación de la luz láser y, al mismo tiempo, permitir su salida después de penetrar a través del medio activo encargado de proporcionar el \enquote{sustrato} necesario para la emisión estimulada. La Figura \ref{fig:1.4} mostraba como, por ejemplo, una manera de liberar la luz láser consiste en utilizar espejos parcialmente transparentes a la frecuencia amplificada. Las vueltas de los fotones atrapados en el interior de la cavidad consiguen multiplicar la energía liberada. 
...
\begin{figure}[htbp]
  ...
  \caption{Ángulo de divergencia $\theta$ del ancho del haz láser $w(z)$ con la longitud $z$, longitud de Rayleigh $z_{R}$, y ancho inicial $w_{0}$. Obsérvese que, para $z=z_{R}$, el ancho del haz aumenta en un factor $\sqrt{2}$.}
  \label{fig:1.11}
\end{figure}
...
El índice angular $l$, tiene tres nombres, algunos de origen matemático: \autocite{Yao2011} \emph{\acrfull{tc}} (carga topológica, el más común), \emph{winding number} (número de vueltas) o \emph{dislocation strength}. Este número concede al frente de onda del campo eléctrico una trayectoria helicoidal en su dirección de propagación $z$, su valor determina el número de vueltas o hélices formadas en un periodo de amplitud $2 \pi$, y su signo proporciona el sentido de giro. Esencialmente, el subíndice $l$ es el número de veces que la luz se \enquote{retuerce} al recorrer una longitud de onda. El momento angular aportado a los fotones (en la última parte del capítulo \S\ref{cap:4}) es consecuencia de este \enquote{alabeo} del frente de onda, apareciendo representada espacialmente la evolución de la fase en la Figura \ref{fig:1.13}.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.28\textwidth]{Figuras/ch1_quartz_plate.png}
  \caption{Montaje de los platos de cuarzo utilizados por Beth. \autocite{Beth1936}}
  \label{fig:1.14}
\end{figure}

Cincuenta años más tarde, en 1992, L. Allen \emph{et al} demostraron \autocite{Allen1992} que el término $\exp(-il \phi)$ en los modos de Laguerre-Gauss de la ecuación \eqref{eq:1.37} implica un \acrshort{oam} superior en un factor $l$ al \acrshort{sam}, de acuerdo con la analogía entre mecánica cuántica y la óptica paraxial \autocite{Born2019} de que, estos modos se corresponden con los autovalores $\pm l \hslash $ del operador momento angular $L_{z}$, tal que $L_{z}u= -\iu \hslash \pdiff_{\phi}u=\pm l \hslash u$. El experimento que propusieron sigue la misma línea que Beth, pero sustituyendo los discos de cuarzo por parejas de lentes cilíndricas.
...
En el caso de una polarización circular, con $\sigma = \pm 1$ (ver Figura \ref{fig:1.13}), o $\sigma = 0$ para la polarización lineal, la ecuación \eqref{eq:1.42} se transforma en
...
\end{equation}
con $z_{R}$ la longitud de Rayleigh. Tomando el producto vectorial con el vector de posición $\symbf{r}$, la densidad de momento angular es
...
Un plasma consiste en un gas altamente ionizado (mediante, por ejemplo, calentamiento), formado entonces por electrones e iones desligados de los átomos y moléculas que formaban el gas\autocite{Thorne2017}. Su presencia en diversos campos de la ciencia y la tecnología ha ido en aumento desde la década de 1950, durante el desarrollo del programa de fusión termonuclear controlada (y, también, de la bomba termonuclear), pasando por la radioastronomía y el estudio de objetos astronómicos extragalácticos como cuásares y púlsares.
...
En este trabajo, el plasma no está formado por el mismo número de electrones e iones, pero cumple la condición de ser \emph{cuasineutral}, es decir, es un plasma formado por dos \enquote{fluidos} con cargas eléctricas opuestas, y en estos casos los efectos macroscópicos (distancias del tamaño del plasma) de la interacción de Coulomb tienden a neutralizarse (de ahí la palabra cuasineutral). Así, puede definirse la densidad de partículas $n$ como $Zn_{i} \simeq n_{e} \equiv n$, con $Z$ la \emph{ionización media} de los iones del plasma, y $n_{s}$ el número de especies por unidad de volumen del plasma. 

Además, la tasa de colisiones electrón-electrón es más elevada que las tasas de colisiones ión-ión, haciendo que los electrones sean termalizados antes, definiendo entonces una temperatura electrónica y una iónica. Después, las colisiones electrón-ión termalizan el plasma 
\begin{equation}\label{eq:1.47}
  v_{ts} \equiv \bigg(\frac{2k_{B}T_{s}}{m_{s}}\bigg)^{1/2},
\end{equation}
---con $k_{B}$ la constante de Boltzmann---, relacionando directamente las velocidades de los iones y los electrones a través de sus masas, teniendo los iones necesariamente velocidades varios órdenes de magnitud inferiores (son más masivos) que las velocidades de los electrones
...
\begin{equation}\label{eq:1.49}
  \omega_{pe} \equiv \bigg(\frac{ne^{2}}{\epsilon_{0}m_{e}}\bigg)^{1/2},
\end{equation}
...
\begin{equation}\label{eq:1.50}
  m_{e} \dvN{\Delta x}{t}{2} = eE = -m_{e} \omega^{2}_{pe} \Delta x.
\end{equation}
...
\begin{equation}\label{eq:1.51}
  \lambda_{D} \equiv \frac{1}{\omega_{pe}}\bigg(\frac{k_{B}T}{m_{e}}\bigg)^{1/2},
\end{equation}
que utilizando la ecuación \eqref{eq:1.49}, es igual a la relación independiente de la masa
\begin{equation}\label{eq:1.52}
  \lambda_{D} \equiv \bigg(\frac{\epsilon_{0}k_{B}T}{ne^{2}}\bigg)^{1/2}.
\end{equation}
...
\begin{equation}\label{eq:1.54}
  r_{c} \equiv \frac{e^{2}}{4 \pi \epsilon_{0}k_{B}T},
\end{equation}
...
Definiendo el \emph{número de Debye} como
\begin{equation}\label{eq:1.56}
  N_{D} \equiv \frac{4 \pi}{3}n \lambda^{3}_{D},
\end{equation}
...
\begin{equation}\label{eq:1.57}
  N_{D} = \frac{\lambda_{D}}{3r_{c}} = \frac{1}{3\sqrt{4 \pi}}\bigg(\frac{r_{d}}{r_{c}}\bigg)^{3/2} = \frac{4 \pi \epsilon^{3/2}_{0}}{3e^{3}}\frac{(k_{B}T)^{3/2}}{n^{1/2}}.
\end{equation}
Cualquier partícula cargada en el plasma sufre un fenómeno de apantallamiento (las partículas de signo opuesto son atraídas) que disminuye la carga efectiva de dicha carga, vista desde el exterior de la esfera definida por este parámetro. Esta reducción de la carga efectiva es conocida como \emph{Debye shielding}. Analizando la ecuación \eqref{eq:1.57}, cuando $N_{D} \ll 1$ las interacciones electrostáticas son dominantes ($r_{d}/r_{c} \ll 1$, fuertemente acoplado), correspondientes con un plasma frío y denso. En cambio, $N_{D} \gg 1$ está asociado con colisiones entre partículas dominantes ($r_{d}/r_{c} \gg 1$, débilmente acoplado), formándose plasmas calientes y dispersos. 
...
\begin{table}[htbp]
  ...
  \toprule
    Plasma & {$n$ (\unit{m^{-3}})} & {$T$ (\unit{eV})} & {$\omega_{pe}$ (rad/s)} & {$\lambda_{D}$ (\unit{m})} & {$N_{D}$} \\
  \midrule
...
Existen diversos tipos de fenómenos ondulatorios asociados a la presencia de campos eléctricos y magnéticos que dependen de la naturaleza del plasma y los propios campos eléctricos y magnéticos con los que interacciona. Esta sección \S\ref{sec:1.2.2}, presenta una teoría linealizada \autocite{Chen2015,Bittencourt2004} (amplitudes de los campos eléctrico y magnético pequeñas) adaptada a un plasma muy caliente (débilmente acoplado), globalmente neutro (cuasineutral), es decir, $\grad \cdot \symbf{E} = 0$ (la densidad total de carga eléctrica es nula a las escalas de estudio del plasma)\autocite{Sureau1995}, y sin magnetización presente en su interior ($\symbf{M}=\symbf{0}$).
...
\end{equation}
siendo $\symbf{u}_{e}$ la velocidad promedio (técnicamente es la velocidad de la \emph{colectividad estadística} formada por los electrones) del conjunto de los electrones que forman el plasma. De acuerdo con la segunda ley de Newton, podemos escribir la ecuación del movimiento de los electrones inducido por el campo eléctrico como $-e\symbf{E}=m_{e}\dot{\symbf{u}}_{e}$. 
...
Por último, hay que mencionar la idea de emplear un haz infrarrojo con \acrshort{oam} para generar armónicos. Los \acrshortpl{hoh} generados también tienen \acrshort{oam}, siendo este mayor cuanto mayor es el orden del armónico. La adición de \acrshort{oam} a un pulso \acrshort{xuv} ultracorto y coherente, producido a partir de una semilla de armónicos, introduce un nuevo \enquote{grado de libertad} en las propiedades de la luz láser que, probablemente descubrirá nuevas aplicaciones en el futuro. La Figura \ref{fig:1.21} representa gráficamente este concepto, representando un chorro de gas argón como medio óptico no lineal.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{Figuras/ch1_betaxrays.pdf}
...
Escogiendo un átomo de un elemento químico determinado, por ejemplo, kriptón o xenón, e ionizándolos hasta conseguir una estructura electrónica idéntica a un elemento químico con el último orbital completo (familias de iones isoelectrónicos), por ejemplo, níquel, (iones $\ce{Kr}^{8+}$ y $\ce{Xe}^{16+}$, ambos con $28$ electrones en su capa de valencia) experimentalmente puede observarse un movimiento hacia energías superiores de los niveles electrónicos que participan en las transiciones ópticas o ultravioletas buscadas. 
...
Originalmente, fue sugerido teóricamente por Zherikin \emph{et al} (1976) y Vinogradov \emph{et al} (1977), que además, demostraron la posibilidad de conseguir una emisión de luz \acrshort{vuv} en ciertas transiciones de iones neonoides ($10$ electrones). La confirmación experimental de luz \acrshort{xuv} tuvo lugar unos años más tarde, en 1985 \autocite{Matthews1985,Rosen1985}, apoyándose en iones de selenio e itrio neonoides y técnicas espectroscópicas para medir las ganancias obtenidas, obteniendo coeficientes de ganancia de \qty{5.5 +-  1.0}{cm^{-1}}.
...
Sin embargo, aunque la ganancia máxima aumenta en el \acrshort{tce} su duración disminuye, reduciendo la longitud de aprovechamiento. La solución suele aprovechar el desacoplamiento de los pulsos anteriores para sincronizar la inversión poblacional con la amplificación del pulso \acrshort{xuv} \autocite{Kozlova2020a}. Habitualmente, muchos equipos sitúan el láser de bombeo formando un ángulo de inclinación respecto al eje del plasma, solución llamada \emph{\acrfull{grip}} \autocite{Keenan2005}.  

Entre los diferentes métodos existentes para producir el plasma amplificador, un posible esquema emplea un pulso láser infrarrojo muy intenso con la potencia necesaria para separar los electrones de los átomos y moléculas de un gas inicial. Este método está basado en la ionización por efecto túnel (\acrshort{ofi}) del medio gaseoso durante su interacción con el haz láser. 

Otra pregunta que aparece en estos esquemas, y en las investigaciones presentadas hasta ahora, es: ¿qué luz está siendo amplificada?, ¿qué está provocando la emisión \acrshort{xuv}? La respuesta a ambas preguntas es el \enquote{ruido}. En realidad, los fotones amplificados han sido emitidos espontáneamente desde el nivel superior de la transición láser, ejerciendo como las semillas necesarias para inducir el efecto resonante entre los estados que participan en la emisión \acrshort{xuv}, de ahí la palabra ruido. Por tanto, estos métodos están basados en la amplificación de la emisión espontánea (ruido) de radiación o, en la literatura especializada, \emph{\acrfull{ase}}.
...
\begin{figure}[htbp]
  ...
  \begin{subcaptionblock}{.8\textwidth}
    \centering
    \includegraphics[width=0.7\textwidth]{Figuras/ch1_hhg_inyec.pdf}
    \caption{Producción del \acrshort{hoh}}\label{fig:ch1_hhg1}
  \end{subcaptionblock}
  \begin{subcaptionblock}{.8\textwidth}
    \centering
    \includegraphics[width=0.7\textwidth]{Figuras/ch1_hhg_amplif.pdf}
...
La Figura \ref{fig:1.24} muestra, de manera simplificada, las dos etapas básicas de la cadena de amplificación. De acuerdo con las estimaciones que realizaron, el coeficiente de ganancia pequeño $g_{0}$ obtenido en el experimento fue de \qty{80}{cm^{-1}}, y la saturación fue alcanzada tras recorrer \qty{1.7}{mm} del canal de plasma. Además, el tiempo de retardo entre la formación del canal de plasma y la inyección del armónico es importante para optimizar la energía obtenida. El intervalo de amplificación comenzaba \qty{5}{ps} después de la interacción del pulso \acrshort{nir} con los \qty{4}{mm} de longitud de la celda de argón, y duraba aproximadamente \qty{8}{ps}. 
...
En este caso, la primera demostración de un láser \acrshort{sxrl} fue realizada por Nagata \emph{et al} (1993) \autocite{Nagata1993}, basándose en la recombinación explicada en la sección \S\ref{sec:1.4} para conseguir la inversión poblacional necesaria en los iones de litio hidrogenoide \ce{Li^+} formados.
...
La demostración experimental fue llevada a cabo por Lemoff \emph{et al} (1995) \autocite{Lemoff1995}, empleando un sistema láser \ce{Ti{:}Zafiro} infrarrojo (\qty{50}{fs}, \qty{50}{mJ}, \qty{10}{Hz}) con iones de xenón paladiumoide \ce{Xe^{8+}} a \qty{41.8}{nm}. El régimen de saturación llegó más tarde, de la mano de Sebban \emph{et al} (2001)\autocite{Sebban2001}, multiplicando por cuatro el coeficiente de ganancia obtenido por Lemoff (\qty{16.8}{cm^{-1}}) y proporcionando \num{5e9} fotones por pulso a \qty{41.8}{nm}, utilizando el mismo tipo de sistema láser \ce{Ti{:}Zafiro} (\qty{330}{mJ}, \qty{35}{fs}, \qty{10}{Hz}) para formar el ión de xenón paladiumoide \ce{Xe^{8+}}.
...
Por otra parte, en un reciente artículo de A. Kabacinski, E. Oliva, F. Tissandier \emph{et al} ($2023$)\autocite{Kabacinski2023}, se ha demostrado un aumento de la energía extraída en la amplificación gracias al control de la velocidad de grupo del pulso \acrshort{nir} de bombeo. Detrás de la propuesta se encuentra el desfase entre las velocidades de grupo del haz \acrshort{nir} de bombeo y la semilla de armónicos \acrshort{hoh} durante su propagación (la velocidad de propagación depende de la longitud de onda en un plasma) a través del plasma, provocando que la extracción de energía disminuya notablemente cuando el armónico adelanta al pulso de bombeo.

La energía final obtenida fue tres veces superior (de $19 \pm 6\%$ hasta $59 \pm 4\%$) cuando la velocidad de grupo del pulso \acrshort{nir} de bombeo aumentó de $0,55c$ hasta $1,05c$. Además, también demostraron que la duración del pulso \acrshort{sxrl} está gobernada por esta velocidad del haz de bombeo. Compensando la diferencia de velocidades es posible mantener la duración del pulso \acrshort{sxrl} constante a lo largo del canal de plasma, consiguiendo duraciones de \qty{350}{fs} con energía de \qty{2}{µJ}.

En la Figura \ref{fig:1.30}, aparecen representadas las mediciones del impacto de la velocidad de grupo sobre la duración del pulso \acrshort{sxrl}, con una densidad electrónica cercana a $n_{e}=\qty{7e19}{cm^{-3}}$, utilizando una técnica desarrollada por A. Kabacinski \emph{et al} ($2022$)\autocite{Kabacinski2022}. Los resultados mostraron distintos comportamientos según el desfase en la velocidad de grupo. Cuando el desfase es fuerte, la duración del pulso aumenta rápidamente con la longitud del chorro de gas. En cambio, si la desincronización es menor, la duración del pulso aumenta lentamente. De esta forma, experimentalmente pudo observarse que a mayor desfase entre las velocidades de grupo, mayor es la duración del pulso \acrshort{sxrl} al propagarse a lo largo del amplificador.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{Figuras/ch1_dur_energ.png}
  \caption{\textbf{a-c}, Evolución de la duración del pulso frente a la longitud del chorro para velocidades de grupo de $1,09c \pm 0,04c$ (\textbf{a}), $1,00c \pm 0,03c$ (\textbf{b}) y $0,98c \pm 0,02c$ (\textbf{c})\autocite{Kabacinski2023}. En azul están representadas las mediciones de la duración del pulso y en rojo su regresión lineal.}
  \label{fig:1.30}
\end{figure}
